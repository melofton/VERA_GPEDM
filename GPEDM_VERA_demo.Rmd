...
# install GPEDM package from GitHub
...
temp <- targets |>
  ...
  tidyr::fill(observation, .direction = "downup")
...
### Visualize autocorrelation lags in target variable
...
First, we will extend the dataframe to include the next 1-35 days so we can supply that as `newdata` to the `fitGP` function below.
```{r}
...
HP_train=HPlags[1:(nrow(HPlags) - forecast_time_horizon),]
HP_test=HPlags[(nrow(HPlags) - forecast_time_horizon + 1):nrow(HPlags),]
```
...
3. *x* is the column names of the lags using in prediction as a vector of character strings
...
```{r}
mod <- fitGP(data = HP_train, 
          y = "observation",
          x = c("observation_1","observation_30","observation_90"),
          time = "Time")
```

### Make empty forecast dataframe
```{r}
...
for(t in 1:nrow(HP_test)){
  pred_list <- predict(mod, newdata = HP_test[t,])
  pred_df <- pred_list$outsampresults
  
  if(t != nrow(HP_test)){
  HP_test[t+1,"observation_1"] <- pred_df$predmean
  }
  
  if(is.na(HP_test[t+1,"observation_30"]) & t != nrow(HP_test)){
    lag_30 <- fc_final %>%
      filter(datetime == forecast_dates[t-29] & parameter == "mu") %>%
      pull(prediction)
    HP_test[t+1,"observation_30"] <- lag_30
  }
...
forecast_file1 <- c(paste0(forecast_file, ".csv"))
write.csv(fc_final, forecast_file1, row.names = FALSE)
